...
def stt(audio):
    ...
    with open(filename, "rb") as audio_file:
        transcript = client.audio.transcriptions.create(model=MODEL_WHISPER, file=audio_file)
    os.remove(filename)
...
def return_answer_candidate(df, query):
    query_embedding = client.embeddings.get_embedding(query, engine="text-embedding-ada-002")
